...
### Model Analysis
以丰富的视图多角度地可视化模型网络各层数据的分布和关键统计信息，便于快速了解当前模型网络设计的合理性，实现快速定位模型异常问题。在使用该功能的步骤如下：
#### 1、随机采样落盘网络节点数据
用户可使用有random dump功能的paddle版本打印每层节点数据，或者自行按给出的数据格式打印
当前paddle1.8.5版本支持随机采样落盘网络节点数据包： https://paddle-qa.bj.bcebos.com/WheelPackages/temp/paddlepaddle-0.0.0-cp27-cp27mu-linux_x86_64.whl

自带random dump功能的paddle使用方法：
```python
join_save_params = []
...
截取部分数据说明如下：
```python
#样本唯一标识\t神经元名称:神经元节点个数:每个神经元的输出值\t神经元名称:神经元节点个数:每个神经元的输出值
