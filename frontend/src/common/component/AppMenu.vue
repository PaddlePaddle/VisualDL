      ...
      <v-toolbar
        ...
        <v-toolbar-items>
          <v-menu
            ...
            > <span class="runs-selected-text">{{ this.keyConfig.kRuns.title }}: {{ runs.length == 0 ? this.keyConfig.kNotRunsSelected.title : runs.join(', ') }} </span>
...
export default {
  ...
  created() {
    ...
    getRuns().then(({errno, data}) => {
      ...
      this.$router.replace( {path: this.initialRoute, query: {runs: this.runs, language: this.language}});
    });
  },
  watch: {
    runs: function(val) {
      if (this.runs) {
        this.$router.push( {query: {runs: this.runs, language: this.language}});
      }
    },
  ...
  methods: {
    handleItemClick: function(item) {
      ...
      this.$router.push( {path: item.url, query: {runs: this.runs, language: this.language}});
    },
