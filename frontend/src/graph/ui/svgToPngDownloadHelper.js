...
(function () {
    let out$ = window;
    let doctype = '<?xml version="1.0" standalone="no"?><!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd" [<!ENTITY nbsp "&#160;">]>';
    ...
    function styles(el, options, cssLoadedCallback) {
        let selectorRemap = options.selectorRemap;
        let modifyStyle = options.modifyStyle;
        let modifyCss = options.modifyCss || function (selector, properties) {
            selector = selectorRemap ? selectorRemap(selector) : selector;
            let cssText = modifyStyle ? modifyStyle(properties) : properties;
                ...
                for (let j = 0, match; j < rules.length; j++, match = null) {
                    ...
                    if (typeof (rule.style) !== 'undefined') {
                        ...
                        if (match) {
                            ...
                            let fontUrlRegexp = /url\(["']?(.+?)["']?\)/;
                            // TODO: This needs to be changed to support multiple url declarations per font.
                            let fontUrlMatch = rule.cssText.match(fontUrlRegexp);

                            let externalFontUrl = (fontUrlMatch && fontUrlMatch[1]) || '';
                            let fontUrlIsDataURI = externalFontUrl.match(/^data:/);
                            if (fontUrlIsDataURI) {
        ...
        function getFontMimeTypeFromUrl(fontUrl) {
            ...
            let extensions = Object.keys(supportedFormats);
        ...
        function processFontQueue(queue) {
            ...
            function processNext(font) {
                ...
                function fontLoaded() {
                    ...
                    let fontBits = oReq.response;
                    let fontInBase64 = arrayBufferToBase64(fontBits);
                ...
                function updateFontStyle(font, fontInBase64) {
                    let dataUrl = 'url("data:' + font.format + ';base64,' + fontInBase64 + '")';
    ...
    out$.prepareSvg = function (el, options, cb) {
        ...
        inlineImages(el, function () {
            ...
            let fos = clone.querySelectorAll('foreignObject > *');
    ...
    out$.svgAsDataUri = function (el, options, cb) {
        out$.prepareSvg(el, options, function (svg) {
            let uri = 'data:image/svg+xml;base64,' + window.btoa(reEncode(doctype + svg));
            if (cb) {
                cb(uri);
            }
        });
    ...
    out$.svgAsPngUri = function (el, options, cb) {
        ...
        let convertToPng = function (src, w, h) {
            let canvas = document.createElement('canvas');
    ...
    out$.download = function (name, uri) {
        if (navigator.msSaveOrOpenBlob) {
            ...
            let downloadSupported = 'download' in saveLink;
            if (downloadSupported) {
                saveLink.download = name;
                saveLink.style.display = 'none';
                document.body.appendChild(saveLink);
                try {
                    let blob = uriToBlob(uri);
    ...
    function uriToBlob(uri) {
        let byteString = window.atob(uri.split(',')[1]);
